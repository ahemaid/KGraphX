...
module.exports = function ( graph ){
    ...
    if ( validURL(url) === false ) {
      ...
      if ( tokens.length === 2 ) {
        ...
        if ( pr.length > 0 ) {
          ...
          if(basePref!="http://www.wikidata.org/entity") {
          url = basePref + name;
        }
      }
        else {
  ...
  editSidebar.checkProperIriChange = function ( element, url ){
    ...
    if ( element.type() === "rdf:type" || element.type() === "owl:disjointWith" ) {
  ...
  editSidebar.updateSelectionInformation = function ( element ){
    ...
    if ( element === undefined ) {
      ...
      let resultHtml = window[element.id()]; 
      if(window[element.id()] &&  resultHtml.tagName !== 'g' && Object.keys(window[element.id()]).length > 0 ){
        console.log(window[element.id()])
        console.log(resultHtml)
        console.log(resultHtml.tagName)
        document.getElementById('search-result-details').innerHTML = window[element.id()]; 
        document.getElementById('setupSideMenu-input-hidden').click(); 
        
      } else {
        document.getElementById('search-result-details').innerHTML = '';
      }
      ...
      d3.select("#element_labelEditor")
        ...
        .on("keydown", function (){
          ...
          if ( d3.event.keyCode === 13 ) {
            d3.event.preventDefault();
            var sanityCheckResult;
            console.log("Element changed Label key down");
            var url = getURLFROMPrefixedVersion(element);
            if ( element.iri() !== url ) {
              if ( elementTools.isProperty(element) === true ) {
                sanityCheckResult = editSidebar.checkProperIriChange(element, url);
    ...
    if ( elementTools.isProperty(selectedElement) ) {
      if ( selectedElement.type() === "owl:DatatypeProperty" )
        ...
        if ( selectedElement.domain() !== selectedElement.range() ) {
          availiblePrototypes.push("rdf:type");
        }
