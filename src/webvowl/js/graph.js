...
module.exports = function ( graphContainerSelector ){
    ...
    dragBehaviour = d3.behavior.drag()
      ...
      .on("dragend", function ( d ){
        ...
        if ( d.type && d.type() === "Class_dragger" ) {
          ...
          // console.log("nX, nY, delayedHider")
          // console.log(classDragger)
          clearTimeout(delayedHider);
  ...
  function addClickEvents(){
    ...
    nodeElements.on("click", function ( clickedNode ){
      ...
      if ( touchDevice === true && doubletap() === true ) {
        ...
        window.newParent = clickedNode;
        // console.log("I am clicked"); 
        // console.log(clickedNode.label()); 
        let resultHtml = window[clickedNode.id()]; 
        // console.log("resultHtml");
        // console.log(resultHtml);
        if(clickedNode.type() === 'owl:Class' && resultHtml &&  resultHtml.tagName === 'g' 
        && clickedNode.label() !== 'NewResource'){
          console.log("I am new node find me please...");
          window.currentClickedNodeId = clickedNode.id()
          let element_search_filter = document.getElementById('search-filter-hidden')
          element_search_filter.value = clickedNode.label();
          localStorage.setItem("search_node", true) 
          element_search_filter.click()
        }
    ...
    nodeElements.on("dblclick", function ( clickedNode ){  
      ...
      if ( editMode === true ) {
        //----------------------------------//
        //----------------------------------//
        //-in case of class double clicked--// 
        //-it open search popup ------------//
        //----------------------------------//
        if(clickedNode.type()==='owl:Class'){
        ...
        window.parentInqueue = clickedNode;
        //----------------------------------//
        //----------------------------------//
        // clickedNode.raiseDoubleClickEdit(defaultIriValue(clickedNode));
      }
    ...
    labelGroupElements.selectAll(".label").on("click", function ( clickedProperty ){
     ...
     /* console.log("label clicked")
      ...
      window.domainId = null;
      window.rangeId = null;
      if(clickedProperty.domain())
        window.domainId = window[clickedProperty.domain().id()+'wikiId'];
      if(clickedProperty.range())
        window.rangeId  = window[clickedProperty.range().id()+'wikiId'];

      if( window.domainId && window.rangeId){
        window.clickedProperty = clickedProperty;
      }
      console.log("clickedProperty.type()");
      console.log(clickedProperty.type());
      if(clickedProperty.type() !== "rdfs:subClassOf"){

        document.getElementById('loadProp-hidden').click();
      }
    ...
    labelGroupElements.selectAll(".label").on("dblclick", function ( clickedProperty ){
      ...
      if ( editMode === true ) {
        ...
        console.log("clickedProperty.type()");	
        console.log(clickedProperty.type());	
        if(clickedProperty.type() !== "rdf:type" && clickedProperty.type() !== "owl:DatatypeProperty"){	
      ...
      if(clickedProperty.type() == "owl:DatatypeProperty") {
        clickedProperty.raiseDoubleClickEdit(defaultIriValue(clickedProperty));
      }
    }
    ...
    labelGroupElements.selectAll(".label").on("drop", function ( clickedProperty ){
      // console.log("I am")
      d3.event.stopPropagation();
      if ( editMode === true ) {
  ...
  graph.changePropertyType = function ( element ){
    ...
    if ( element.backupLabel() !== undefined ) {
      ...
      // console.log("graph.js changepropertytype")
      aProp.label("newObjectProperty");
    }
    
    if ( aProp.type() === "rdf:type" ) {
  ...
  $( '.updateProperty' ).on( 'click', function (div) { 
    ...
    console.log(div);
    console.log(div.target.innerText);
    console.log("window.clickedProperty");
    console.log(window.clickedProperty);
    console.log(window.clickedProperty.id());
    let property_name = div.target.innerText;
    window.clickedProperty.label(property_name);
    window.clickedProperty.iri(':'+property_name.replace(/ /g,"_"));
 });
  //////////-----------------------------------------//////////////
  //////////-----------------------------------------//////////////
  function createNewNodeAtPosition( pos ){
    ...
    let selected_object = localStorage.getItem("selected_object");
    let selected_object_wikiid = localStorage.getItem('selected_object_wikiid');
    if(selected_object) {
      typeToCreate = "owl:Class";
    }
    ...
    if ( typeToCreate === "owl:Thing" ) {
      ...
      if(selected_object){
        ...
        aNode.label(selected_object);
        if(selected_object_wikiid)
        aNode.iri("wd:" + selected_object_wikiid.replace(/ /g,"_"));
        else
        aNode.iri(selected_object.replace(/ /g,"_"));
        setIRI = true;
      } else { 
        aNode.label("NewResource");
      }
    ...
    if(selected_object_wikiid){
      window[aNode.id() +'wikiId'] = selected_object_wikiid;
      localStorage.removeItem('selected_object_wikiid');
    }
    //-----------------------------------------------------//
    //---------Logic to Handle link bw class objects ------//
    //-----------------------------------------------------// 
    let parentHtml;
    if(window.newParent){
      parentHtml = document.getElementById(window.newParent.id());
    }
    console.log("selected_object,parentHtml removing 1")
    console.log(selected_object,parentHtml)
    if(selected_object && parentHtml){
      ...
      var draggerEndPos = [nX, nY];
      createNewObjectProperty(window.newParent, classDragger.parentNode(), draggerEndPos);
    } else { 
      window.parentInqueue = classDragger.parentNode();
  ...
  graph.genericPropertySanityCheck = function ( domain, range, typeString, header, action ){
    if ( domain === range && typeString === "rdf:type" ) {
      graph.options().warningModule().showWarning(header,
        "rdf:type can not be created as loops (domain == range)",
        action, 1, false);
      return false;
    }
  ...
  graph.propertyCheckExistenceChecker = function ( property, domain, range ){
    var allProps = unfilteredData.properties;
    var i;
    if ( property.type() === "rdf:type" || property.type() === "owl:disjointWith" ) {
  ...
  function createNewObjectProperty( domain, range, draggerEndposition ){
    ...
    let selected_object_type = localStorage.getItem('selected_object_type');
    let selected_object_type_id = localStorage.getItem('selected_object_type_id');
    if(selected_object_type == 'instance-of'){
      defaultPropertyName = "rdf:type";
    }
    var propPrototype = PropertyPrototypeMap.get(defaultPropertyName.toLowerCase());
    var aProp = new propPrototype(graph);
    aProp.id("Property" + eP++);
    aProp.domain(domain);
    aProp.range(range);
    // console.log("graph.js create new object prop")
    
    var defaultPropertyName = d3.select("#defaultProperty").node().title;
    if(selected_object_type){
      localStorage.removeItem('selected_object_type');
      if(selected_object_type == 'instance-of'){
        aProp.label("isTypeOf");
      } else {
        aProp.label(selected_object_type);
        if(selected_object_type_id) {
        localStorage.removeItem('selected_object_type');
        aProp.iri('wdt:'+selected_object_type_id.replace(/ /g,"_"));
        }
        else {
        aProp.iri(':'+selected_object_type.replace(/ /g,"_"));
        }
      }
    } else {
        
      if(defaultPropertyName == 'rdf:type' || selected_object_type == 'instance-of'){
    ...
    aProp.labelObject().increasedLoopAngle = true;
     // not selecting in case of creation from right side menu
    if(!selected_object_type){
      
      // aProp.enableEditing(autoEditElement); 
    } else {
      options.focuserModule().handle(domain, true);
      // domain.enableEditing(autoEditElement);
  ...
  graph.createDataTypeProperty = function ( node ){
    ...
    //   aProp.label("HasRelation");
    //  } else if (defaultPropertyName === 'rdfs:subClassOf') {

    //   aProp.label("isTypeOf");
    //  }
    // console.log("aNode graph.js")
    // console.log(aNode)
    // console.log(node)
  ...
  graph.modified_dblClickFunction = function (){
    ...
    // console.log("here createNewNodeAtPosition and removing3")
    // localStorage.removeItem("selected_object");
  ...
  graph.modified_dblTouchFunction = function ( d ){
    ...
    // console.log("modified_dblTouchFunction createNewNodeAtPosition")
    createNewNodeAtPosition(grPos);
  };
